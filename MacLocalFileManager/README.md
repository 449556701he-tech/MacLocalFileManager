...
MacLocalFileManager 是一个 no-LLM 的 macOS 本地中文文件搜索工具。v1.1 已把 beta1.0 的 Spotlight 风格窗口接入 macOS 日常入口：菜单栏常驻、全局快捷键、关闭后后台保留。
...
- v1.1 支持菜单栏常驻、显示/隐藏搜索窗口，以及全局快捷键 `⌃F` 唤起。
...
- 全局快捷键只在本机监听键盘事件；如果 macOS 阻止监听，程序会继续运行并在状态栏提示，不会上传任何输入内容。
...
- 启动后会在 macOS 菜单栏常驻一个搜索图标。
- 点击菜单栏图标会打开白底圆角快捷面板，可显示/隐藏搜索、打开扫描设置、刷新索引或退出；这里不用 Qt 原生托盘菜单，以避开当前 macOS 上的原生菜单崩溃问题。
- 默认全局快捷键是 `⌃F`，避免抢占系统 Spotlight / Finder 搜索、输入法切换等常见系统快捷键。
- 如果快捷键没有生效，可在“系统设置 -> 隐私与安全性 -> 辅助功能”里允许本 App；不授权也不影响菜单栏和普通搜索。
- 可在“设置 -> 显示选项”里关闭全局快捷键，设置会在下次启动后生效。
...
dist/MacLocalFileManager-v1.1.0.dmg
```
...
dist/MacLocalFileManager-English-v1.1.0.dmg
```
