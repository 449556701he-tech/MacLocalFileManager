...
REASON_IMAGE_SIMILARITY = "相似图片命中"
...
class SemanticSearcher:
    def __init__(
        ...
        image_similarity_backend: BaseEmbeddingBackend | None = None,
    ) -> None:
        self.db = db
        self.backend = backend or DeterministicTextEmbeddingBackend()
        self.image_backend = image_backend or create_default_image_embedding_backend(db)
        self.image_similarity_backend = image_similarity_backend or create_default_image_similarity_backend()
    ...
    def search_similar_image(self, image_path: str, limit: int = 20, category: str = CATEGORY_ALL) -> list[SearchResult]:
        if not self.db.get_bool_setting(SEMANTIC_ENABLED_SETTING, False):
            return []
        query_vector = self.image_similarity_backend.embed_image(image_path)
        hits = self.store.search(
            self.image_similarity_backend,
            query_vector,
            limit=limit * 4,
        ...
        for hit in hits:
            file_row = self.db.get_file(hit.file_id)
            if file_row is None or file_row["path"] == image_path:
                continue
            result = semantic_hit_to_result(file_row, hit, category, REASON_IMAGE_SIMILARITY, match_type="相似图片")
            if result is None:
                continue
            existing = results_by_file.get(result.id)
            if existing is None or result.rank < existing.rank:
...
def semantic_hit_to_result(file_row, hit, category: str, reason: str, match_type: str = "语义命中") -> SearchResult | None:
